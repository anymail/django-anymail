...
Note that this setting also affects inbound webhooks, which must call Mailtrap
APIs to retrieve inbound messages. The default for inbound webhooks is
``"https://mailtrap.io/api/"``. If you are using both inbound email and
sending, it's best to *not* set this setting and instead supply an `api_url`
option for sending in Django's :setting:`MAILERS` configuration or a call to
:func:`django.core.mail.get_connection`.

.. setting:: ANYMAIL_MAILTRAP_TRACKING_SECRET

.. rubric:: MAILTRAP_TRACKING_SECRET

The Mailtrap webhook signing secret for tracking (email sending) webhooks.
This is generated by Mailtrap after you create a webhook.

.. setting:: ANYMAIL_MAILTRAP_INBOUND_SECRET

.. rubric:: MAILTRAP_INBOUND_SECRET

The Mailtrap webhook signing secret for inbound webhooks. This is generated by
Mailtrap after you create a webhook.
...
Mailtrap supports signed webhook events. You can configure Anymail's tracking
webhook to verify Mailtrap's signature, require a :setting:`WEBHOOK_SECRET
...
To configure an event tracking webhook, in Mailtrap's "Add new webhook" screen
enter:
...
* Webhook URL:
    ...
    :samp:`https://{yoursite.example.com}/anymail/mailtrap/tracking/`
...
After you have created the webhook, the signing secret will become available in
Mailtrap's webhooks list---see `Getting your signing secret`_ in Mailtrap's
documentation. To enable signature verification, copy the secret and provide it
as Anymail's :setting:`MAILTRAP_TRACKING_SECRET
<ANYMAIL_MAILTRAP_TRACKING_SECRET>` in your Django settings:

.. code-block:: python

    ANYMAIL = {
        # ...
        "MAILTRAP_TRACKING_SECRET": "<signing secret from Mailtrap>",
    }
...
.. _Getting your signing secret:
    https://docs.mailtrap.io/email-api-smtp/advanced/webhooks#getting-your-signing-secret
...
inbound inbox`_. Then configure a Mailtrap webhook to notify Anymail of new
inbound messages. In their "Add new webhook" screen enter:

* Webhook URL:

    Using Anymail's shared webhook secret is optional if you enable Mailtrap
    webhook signature verification. If you are *not* using a shared secret,
    enter:

    :samp:`https://{yoursite.example.com}/anymail/mailtrap/inbound/`

    Or if you *are* using Anymail's :setting:`WEBHOOK_SECRET <ANYMAIL_WEBHOOK_SECRET>`,
    include the *random:random* shared secret in the URL:

    :samp:`https://{random}:{random}@{yoursite.example.com}/anymail/mailtrap/inbound/`

    where *yoursite.example.com* is your Django site.

* Payload format: JSON (*not* JSON Lines)

* Select area: Inbound inboxes. You can choose "Account-wide (all inboxes)"
  or a specific inbox.

After you have created the webhook, the signing secret will become available in
Mailtrap's webhooks list---see `Getting your signing secret`_ in Mailtrap's
documentation. To enable signature verification, copy the secret and provide it
as Anymail's :setting:`MAILTRAP_INBOUND_SECRET
<ANYMAIL_MAILTRAP_INBOUND_SECRET>` in your Django settings. (Note that inbound
and tracking webhooks use different secrets, so have different setting names.)

The inbound webhook must retrieve inbound message data from Mailtrap's API.
Ensure your Anymail settings include a :setting:`MAILTRAP_API_TOKEN
<ANYMAIL_MAILTRAP_API_TOKEN>` that has at least "viewer" permission for the
selected inbox(es) in the "Inbound" section (as well as "admin" permission
on any sending domains, if you are also sending through Mailtrap).
...
.. code-block:: python
    ...
    ANYMAIL = {
        ...
        "MAILTRAP_API_TOKEN": "<token with inbound viewer permission>",
        # Include to enable Mailtrap webhook signature verification:
        "MAILTRAP_INBOUND_SECRET": "<signing secret from Mailtrap>",
    }

In your inbound signal receiver, the :attr:`event.esp_event
<.AnymailInboundEvent.esp_event>` will be set to the result of Mailtrap's `get
an inbound message`_ API. The ``"inbox_id"`` and ``"id"`` (inbound message id)
fields may be useful for (e.g.,) calling Mailtrap's API to delete the inbound
message after you have processed it

.. _Mailtrap inbound inbox: https://docs.mailtrap.io/inbound-email/overview
.. _get an inbound message:
